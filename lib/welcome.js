...
function buildWelcomeMessageContent(text) {
  return {
    interactiveMessage: {
      body: { text },
      footer: { text: `Created by ${config.ownerName || 'Justine Louise'}` },
      nativeFlowMessage: { buttons: [] }
    }
  }
}

export async function handleGroupParticipantsUpdate(conn, { groupJid, participants, action }) {
  ...
  let metadata = null
  try {
    metadata = await getGroupMetadata(groupJid, conn)
  } catch (e) {
