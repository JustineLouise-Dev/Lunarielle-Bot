...
export async function convertWebpToPng(webpBuffer) {
  ...
  if (sharp) {
    try {
      ...
      console.error('[STICKER] sharp gagal konversi webp->png, fallback ke ffmpeg:', e.message);
    }
  }

  const inputPath = tempFilePath('webp');
...
export async function convertWebpToGif(webpBuffer) {
  const inputPath = tempFilePath('webp');
  const outputPath = tempFilePath('gif');
  ...
  try {
    ...
    await execFileAsync(ffmpegPath, [
      ...
      '-i', inputPath,
      '-vf',
      `fps=${ANIMATED_FPS},scale=${MAX_STATIC_SIZE}:${MAX_STATIC_SIZE}:force_original_aspect_ratio=decrease:flags=lanczos,` +
