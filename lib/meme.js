...
async function supportsDrawtext(ffmpegPath) {
  try {
    const { stdout } = await execFileAsync(ffmpegPath, ['-filters']);
    return /drawtext/.test(stdout);
  } catch {
    return false;
  }
}
...
async function resolveFfmpegPath() {
  ...
  for (const candidate of candidates) {
    ...
    if (await supportsDrawtext(candidate)) {
  ...
  throw new Error(
    'Tidak ditemukan ffmpeg yang mendukung filter drawtext (butuh build dengan libfreetype).\n' +
      'Di Termux: pkg install ffmpeg\n' +
      'Di Debian/Ubuntu: sudo apt install ffmpeg\n' +
      'Atau set FFMPEG_PATH ke lokasi binary ffmpeg yang mendukung drawtext.'
  );
}
