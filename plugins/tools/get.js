...
import { getQuotedPlainText } from '../../lib/mediaHelper.js'
...
async function sendAsFile(m, buffer, fileName, mimetype, caption) {
    return m.reply({ document: buffer, mimetype, fileName, caption })
}
...
export default {
    ...
    async execute(m, { args }) {
        ...
        if (!url) {
            ...
            if (quotedText) {
                const urls = quotedText.match(/https?:\/\/[^\s<>"']+/gi)
                if (urls) url = urls[0]
            }
        }
        ...
        try {
            ...
            if (/^image/.test(mime)) {
                if (ext === 'webp') {
                    return m.reply({ sticker: buffer })
                }
                return m.reply({ image: buffer, mimetype: mime, caption })
            }
            ...
            if (/^video/.test(mime)) {
                ...
                return m.reply({ video: buffer, mimetype: mime, caption })
            }

            if (/^audio/.test(mime)) {
                return m.reply({ audio: buffer, mimetype: mime, ptt: false })
            }

            if (/^text|json|javascript|xml/.test(mime)) {
                const text = buffer.toString('utf-8')
                const header = `${caption}\n\n`
                if ((header + text).length > 4000) return sendAsFile(m, buffer, `result.${ext}`, mime, caption)
                return m.reply(header + text)
            }

            return sendAsFile(m, buffer, `file_${Date.now()}.${ext}`, mime, caption)
