...
export default {
    ...
    description: '◈ Lunarille Music Player (Gambar + Audio)',
    execute: async (m, { sock }) => {
        ...
        // Jika ada error, kirim pesan peringatan
...
<img class="musicCoverImg" id="musicCoverImg" src="${imageSrc}" alt="Cover">
...
'0:00'
        ...
        // Payload rich response WhatsApp punya batas ukuran ketat.
        // Audio base64 yang besar (>~500KB) sering bikin server nolak
        // publish dengan MessagePublishNackError (error=479).
        const payloadSizeKB = Buffer.byteLength(base64Data, 'utf8') / 1024;
        if (payloadSizeKB > 700) {
            await sock.message.send(targetChat, {
                text: `⚠️ *Payload terlalu besar (${payloadSizeKB.toFixed(0)} KB)!*\n\nFile audio \`menu.mp3\` terlalu besar untuk dikirim sebagai rich response HTML (limit aman ±700KB). WhatsApp akan menolak pesan ini.\n\n📌 Solusi: kompres/perkecil \`src/menu.mp3\` (idealnya di bawah 400KB, format mp3 bitrate rendah, durasi pendek), lalu coba lagi.`
            });
            return;
        }
        ...
        try {
                    ...
                    message: {
                        richResponseMessage: {
                            submessages: [
                                {
                                    messageType: 0,
                                    messageText: "◈ LevviCode Music Player"
                                }
                            ],
                            messageType: 0,
                            unifiedResponse: {
            ...
            await sock.message.send(targetChat, {
                text: `❌ Gagal mengirim Star Media: ${err?.message || err}`
            });
        }
    }
};
